...
namespace PlantAll
{
    ...
    public partial class ModEntry
    {
        ...
        {
            if (!Config.EnableMod || !__result || !SHelper.Input.IsDown(Config.ModButton) || !(item is Object) || ((Object)item).Category != -74 || item.Stack <= 0)
                return;
            ...
            foreach(var p in placeables)
            {
                ...
                if (Game1.player.CurrentItem == null || Game1.player.CurrentItem.Category != -74 || Game1.player.CurrentItem.Stack <= 0)
                    return;
            }
        ...
        private static void GetPlaceable(int x, int y, List<Point> placeables)
        {
            ...
            {
                if (!Game1.player.CurrentItem.canBePlacedHere(Game1.player.currentLocation, new Vector2(tiles[i].X, tiles[i].Y)) || !Utility.isViableSeedSpot(Game1.player.currentLocation, new Vector2(tiles[i].X, tiles[i].Y), Game1.player.CurrentItem))
                {
                    tiles.RemoveAt(i);
                }
                else
